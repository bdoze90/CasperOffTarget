...
void OnTargets::LoadTargetQuery(std::string query_file) {
	std::ifstream fileop(query_file);
	while (fileop) {
		gRNA *query = new gRNA;
		std::string gcsline;
		std::getline(fileop, gcsline);
		if (gcsline.size() <= 1) {
			break;
		}
		std::vector<std::string> gcs = Msplit(gcsline, ',');
		std::string sequence;
		if (is_compressed) {
...
std::vector<std::string> OnTargets::Msplit(const std::string &text, char sep) {
