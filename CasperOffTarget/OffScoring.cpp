...
void OffScoring::loadCspr(csprRef *r, string cspr_file, string casper_file) {
	ref = r;
	fillHsumatrix(cspr_file, casper_file);
}
...
for (auto it = offs.begin(); it != offs.end(); ++it) {
	...
	if (id > ref->multiStart()) {
		...
		for (int j = 0; j < curmultis.size(); j++) {
			...
			myoff.sequence = S.decompress(the_tailseq, sequence_length-seed_length) + S.decompress(c_base_seq, seed_length);
		...
		myoff.sequence = S.decompress(ref->AccessRefString()->substr(id * 8, 8), sequence_length);  // find out the actual location of the id and get the string there
