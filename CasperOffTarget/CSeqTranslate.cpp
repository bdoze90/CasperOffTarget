...
/* Takes a nucleotide string and turns it into its compressed version. */
...
/* Takes in a compressed location string and returns the */
long SeqTranslate::decompress_location(std::string input) {
    long output=0;
    for (int i=0; i<input.size(); i++) {
